...
class UserContributionService implements UserContributionInterface, TransactionDataGroupMgt
{
    ...
    public function getContributionsByItem($payment_item)
    {
        ...
        return new UserContributionCollection(
            $paginated_contribution,
            $total_contribution,
            $total_balance,
            $unpaid_durations,
    ...
    public function filterContributions($request)
    {
        ...
        //     $usersWhoContributed = collect($contributions->get())->map(function ($e) {
        //         return $e->user_id;
        //     })->toArray();

        //     $usersNotContributed = User::whereNotIn('id', $usersWhoContributed)->get();

        //     return new UserNotContributedResource($usersNotContributed->toArray(), $payment_item);
        // } else {
    ...
    public function getMemberContributedItems($user_id, $year)
    {
        ...
        foreach ($contributions as $contribution) {
            $contributedItem = new MemberContributedItemResource(
                $contribution->id,
                $contribution->payment_item_id,
                $contribution->payment_item_amount,
                $contribution->name,
                $contribution->amount_deposited,
            ...
            if ($reg->frequency == RegistrationFrequency::YEARLY) {
                foreach ($sessions as $session) {
                    ...
                    if (is_null($reg_session)) {
                        ...
                        $reg_debts[] = new MemberPaymentItemResource(
                            $reg->id,
                            'Registration',
                            $reg->amount,
                            $reg->amount,
    ...
    private function getAllMemberContribution($user_id, $year)
    {
        return DB::table('user_contributions')
            ...
            ->where('user_contributions.session_id', $year)
            ->select(
                'payment_items.id as payment_item_id',
                'payment_items.name',
                'payment_items.compulsory',
                'payment_items.amount as payment_item_amount',
                'payment_items.description',
                'payment_items.type',
                'payment_items.frequency',
    ...
    private function getRegistration($user_id)
    {
        ...
        foreach ($reg as $value) {
            $registrations[] = new MemberContributedItemResource(
                $value->id,
    ...
    private function verifyItemPaymentByYear($item, $user_id, $current_session)
    {
        ...
        if (count($this->verifyCompleteItemPaymentByYear($item->id, $user_id, $current_session)) == 0) {
            $to_be_paid = new MemberPaymentItemResource(
                $item->id,
                $item->name,
                $item->amount,
                $item->amount,
    ...
    public function getActivityContributions($request)
    {
        $payment_item = PaymentItem::find($request->payment_item_id);
